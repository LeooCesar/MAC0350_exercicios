...
@app.post("/users")
async def adicionar_usuario(user: User):
    user.id = len(users)
    users.append(user)
    return {"message": f"{user} foi adicionado"}

@app.get("/users")
async def retorna_users(index: int = None):
...
async def delete_user():
    users.clear()
    return {"message": "Todos os usuários foram apagados"}
